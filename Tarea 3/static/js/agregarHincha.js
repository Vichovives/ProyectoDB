//Menu de selección para "Deporte(s) que interesa apoyar".
fetch('static/js/deportes.txt')
    .then(response => response.text())
    .then(data => {
        let lineas = data.split('\n').filter(Boolean);
        let container = document.getElementById('menuDeportes');
...
function actualizarOpcionesSeleccionadas() {
    let checkboxes = [...document.querySelectorAll('#menuDeportes input[type="checkbox"]:checked')];
    opcionesSeleccionadas = checkboxes.map(checkbox => checkbox.value);
}
document.getElementById('menuDeportes').addEventListener('change', actualizarOpcionesSeleccionadas);

//Menu de selección de regiones y comunas
...
//Validador transporte
const validadorTransporte = () => {
...
let formulario = document.forms["agregarHinchaContenedorFoms"];
...
const validarFormulario = () => {
    ...
    let validacionDeportes = document.getElementById("errorDeportes");
    if (!validadorDeporte()) {
        esValido = false;
        validacionDeportes.hidden = false;
    }
    else {
        validacionDeportes.hidden = true;
    }
    ...
    let validacionTransporte = document.getElementById("errorTransporte");
    if (!validadorTransporte()) {
        esValido = false;
        validacionTransporte.hidden = false;
    }
    else {
        validacionTransporte.hidden = true;
    }
...
document.getElementById("confirmarRegistro").addEventListener("click", function() {
    document.getElementById("agregarHinchaContenedorFoms").submit();
});
